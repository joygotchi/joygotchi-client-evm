                    ...
                    <div className="col-end-7 col-span-3 ">
                        <section className="message -left" >
                            <div className="nes-balloon from-left" style={{ padding: '2px' }}>
                                <p>{ownPet && ownPet[1] == 4 ? "Buy Holy water" : <CountDownTimer targetDate={countDownseconds} />}</p>
                            </div>
                        </section>
                    </div>
                                ...
                                <div className="col-start-1 col-end-7 " key={pet.id}>
                                    <div className="flex justify-center ">
                                        <Image
                                            ...
                                            src={`/gotchi/animation/${pet.attr && pet.attr[1] == 0 ? "black_dragon" : "green_dragon"}/${pet.evol && pet.evol[1]}.gif`}
                                        />
                                    </div>

                                    <div className="flex justify-center pt-4">

                                        <p className="font-semibold  leading-none text-default-600 text-center">{pet && pet.label} #{pet && pet.value} - {pet && pet.attr[4] == "0" ? "male" : pet && pet.attr[4] == "1" ? "female" : ""}</p>
                                ...
                                {itemData && itemData.map((item: any) => (
                                    <Tab key={item.id} title={
                                        <Image
                                            width={20}
                                            radius={"none"}
                                            src={`/gotchi/Assets/${item.name == "Beef" ? "Item_Beef" : item.name == "Water" ? "Item_Water" : item.name == "Shield" ? "Item_Shield" : item.name == "Holy Water" ? "Item_HolyWater" : item.name == "Moon Stone" ? "Item_Moonstone" : ""}.png`}
                                        />}>
                                        <Card className="p-3">
                                            <CardBody>
                                                <div className="grid grid-cols-2 gap-2  ">
                                                    <div className="col-span-1 justify-self-start pb-2">USE 1 {item.name} </div>
                                                    <div className="col-span-1 justify-self-end">{item.price.toString()} {item.price} $Token</div>
                                                </div>
