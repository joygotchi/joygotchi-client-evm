...
export const Activity = () => {
            ...
            case "time":
                return (
                    <div className="relative flex justify-end items-center gap-2">
                        <p className="text-bold text-sm capitalize">{(new Date(data.time * 1000)).toDateString()}</p>
                    </div>
                );
    ...
    const getPetList = async () => {
        let response: any = await fetch(`${process.env.HOST}/api/bnb/activity?page=${page}&address=${process.env.NFT_ADDRESS}`)
        ...
        for (const nft of NftListFilter) {
            ...
            nftList.push({ pet: pet, id: nft.id, time: nft.time.toString(), method: nft.data.method })
        }
